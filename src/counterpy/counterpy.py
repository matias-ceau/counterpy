...
def single_point_crossover(a: Sequence, b: Sequence) -> Tuple[Sequence, Sequence]:
    ...
    a1, a2, b1, b2 = list(a[0:p]), list(b[p:]), list(b[0:p]), list(a[p:])
    A = np.hstack((a1, a2))
    B = np.hstack((b1, b2))
    assert assert_type(A, Sequence)
    assert assert_type(B, Sequence)
    return A, B
...
{pp}(/{max_fit})
        ...
        for i in range(int(len(bad_pop) / 2) - 1):
...
def _parser():
    ...
    parser.add_argument(
        "-l", "--length", default=16, type=int, help="Desired cantus length."
    )

    parser.add_argument(
        "-p", "--population", default=20, type=int, help="Population size."
    )
    ...
    parser.add_argument(
        "-r", "--rate", default=1.0, type=float, help="Crossover mutation rate."
    )
    ...
    parser.add_argument(
        "-o", "--octave", default=3, type=int, help="Octave for key center"
    )
